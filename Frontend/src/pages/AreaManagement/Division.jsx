...
const CREATE_DIVISION_URL = `${API_BASE_URL}/api/create-division/`;
...
export default function Division() {
  ...
  const divisionCount = useMemo(() => divisions.length, [divisions]);
  ...
  const fetchDivisions = useCallback(async () => {
    ...
    setErrorMessage("");
    try {
      const response = await fetch(LIST_DIVISION_URL, {
        headers: { Accept: "application/json" },
      });
      const result = await parseResponseSafely(response);
      if (!response.ok)
        throw new Error(getBackendError(result, "Failed to fetch divisions."));
      setDivisions(extractListFromResponse(result));
    } catch (error) {
      setErrorMessage(error.message || "Unable to load divisions.");
      setDivisions([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!divisionName.trim())
      return setErrorMessage("Please enter Division Name.");

    setIsSubmitting(true);
    try {
      const response = await fetch(CREATE_DIVISION_URL, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(
          getBackendError(result, "Failed to create division.", [
            "province",
            "zone",
            "division_name",
          ]),
      ...
      setErrorMessage(error.message || "Failed to create division.");
    } finally {
      setIsSubmitting(false);
    }
  }, [provinceId, zoneId, divisionName, fetchDivisions]);
  ...
  useEffect(() => {
    ...
    setState({
      title: "Division",
      subtitle: "Create and manage divisions under zones",
    });
  ...
  return (
    <PageShell
      ...
      badge="D"
      title="New Division"
      subtitle="Create division against selected province and zone"
    >
      <div className="space-y-5">
          ...
          <form onSubmit={handleFormSubmit}>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4 items-end">
              ...
              <InputField
                ...
                placeholder="e.g. Rawalpindi Division"
                isLight={isLight}
              />
        ...
        <FormSection title="Division list" surface={surface} isLight={isLight}>
            ...
            <table className="min-w-full">
              <thead className={cn(isLight ? "bg-slate-50" : "bg-[#1a2236]")}>
                <tr>
                  ...
                  <TableHead textSoft={textSoft}>Division Name</TableHead>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  ...
                  divisions.map((item, index) => (
                    <tr
                      key={item.id ?? `${item.division_name}-${index}`}
                      className={cn(
                        "border-t",

...
const CREATE_TEHSIL_URL = `${API_BASE_URL}/api/create-tehsil/`;
...
export default function Tehsil() {
  ...
  const tehsilCount = useMemo(() => tehsils.length, [tehsils]);
  ...
  const fetchZonesByProvince = useCallback(async (selectedProvinceId) => {
    if (!selectedProvinceId) return setZones([]);
    ...
    try {
      ...
      const result = await parseResponseSafely(response);
      if (!response.ok) return setZones([]);
  ...
  const fetchDivisionsByZone = useCallback(async (selectedZoneId) => {
    if (!selectedZoneId) return setDivisions([]);
    ...
    try {
      ...
      const result = await parseResponseSafely(response);
      if (!response.ok) return setDivisions([]);
  ...
  const fetchDistrictsByDivision = useCallback(async (selectedDivisionId) => {
    if (!selectedDivisionId) return setDistricts([]);
    setIsLoadingDistricts(true);
    try {
      const response = await fetch(
        buildFilteredUrl(LIST_DISTRICT_URL, "division", selectedDivisionId),
        { headers: { Accept: "application/json" } },
      );
      const result = await parseResponseSafely(response);
      if (!response.ok) return setDistricts([]);
  ...
  const fetchTehsils = useCallback(async () => {
    ...
    setErrorMessage("");
    try {
      const response = await fetch(LIST_TEHSIL_URL, {
        headers: { Accept: "application/json" },
      });
      const result = await parseResponseSafely(response);
      if (!response.ok)
        throw new Error(getBackendError(result, "Failed to fetch tehsils."));
      setTehsils(extractListFromResponse(result));
    } catch (error) {
      setErrorMessage(error.message || "Unable to load tehsils.");
      setTehsils([]);
    } finally {
      setIsLoading(false);
    }
  ...
  }, [divisionId, fetchDistrictsByDivision]);
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!districtId) return setErrorMessage("Please select District.");
    if (!tehsilName.trim()) return setErrorMessage("Please enter Tehsil Name.");

    setIsSubmitting(true);
    try {
      const response = await fetch(CREATE_TEHSIL_URL, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(
          getBackendError(result, "Failed to create tehsil.", [
            "province",
            "zone",
            "division",
            "district",
      ...
      setSuccessMessage("Tehsil created successfully.");
      setProvinceId("");
      setZoneId("");
      setDivisionId("");
      setDistrictId("");
      ...
      setErrorMessage(error.message || "Failed to create tehsil.");
    } finally {
      setIsSubmitting(false);
    }
  }, [provinceId, zoneId, divisionId, districtId, tehsilName, fetchTehsils]);
  ...
  useEffect(() => {
    ...
    setState({
      title: "Tehsil",
      subtitle: "Create and manage tehsils under districts",
    });
  ...
  return (
    <PageShell
      ...
      badge="T"
      title="New Tehsil"
      subtitle="Create tehsil against selected province, zone, division and district"
    >
      <div className="space-y-5">
        <FormSection title="Tehsil details" surface={surface} isLight={isLight}>
          <form onSubmit={handleFormSubmit}>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6 items-end">
              ...
              <SelectField
                ...
                isLight={isLight}
                disabled={!divisionId || isLoadingDistricts}
              ...
              >
                {isSubmitting ? "Adding..." : "Add Tehsil"}
              </button>
            </div>
            <StatusMessages
        ...
        <FormSection title="Tehsil list" surface={surface} isLight={isLight}>
            ...
            <table className="min-w-full">
              <thead className={cn(isLight ? "bg-slate-50" : "bg-[#1a2236]")}>
                <tr>
                  ...
                  <TableHead textSoft={textSoft}>District</TableHead>
                  <TableHead textSoft={textSoft}>Tehsil Name</TableHead>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  ...
                  tehsils.map((item, index) => (
                    <tr
                      key={item.id ?? `${item.tehsil_name}-${index}`}
                      className={cn(
                        "border-t",
                    ...
                    >
                      ...
                      <TableCell>{display(getTehsilName(item))}</TableCell>
                    </tr>
                  ))
                )}
              </tbody>

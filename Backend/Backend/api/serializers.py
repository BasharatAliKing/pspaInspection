...
class BOQSerializer(serializers.ModelSerializer):
    ...
    forInspection = BOQInspectionSerializer(required=False)
    ...
    def create(self, validated_data):
        ...
        inspection_data = validated_data.pop("forInspection", None)

        boq = BOQ.objects.create(**validated_data)

        # create single inspection
        if inspection_data:
            BOQInspection.objects.create(boq=boq, **inspection_data)
